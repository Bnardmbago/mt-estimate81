...
export default function DiscountSettingsPanel() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setRatePercent(String(Math.round(data.estimate_discount_rate * 100)));
      } catch (loadError) {
  ...
  async function handleSave(event: FormEvent) {
    ...
    try {
      const data = await apiJson<DiscountSettings>("/admin/discount-settings", {
        method: "PATCH",
        body: JSON.stringify({ estimate_discount_rate: parsed / 100 }),
      });
      setRatePercent(String(Math.round(data.estimate_discount_rate * 100)));
      setSaved(true);
    } catch (saveError) {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-4">
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/50">
        ...
        <ul className="mt-3 list-disc space-y-1.5 pl-5 text-sm leading-relaxed text-slate-600 dark:text-slate-400">
